...
class Image:
	...
	def write(self, file):
		f = open(file, "wb");
		w = png.Writer(width = self.width, height = self.height, alpha = (self.color_depth == 4));
		w.write(f, self.pixels);
		f.close();

	def stats(self):
		src = "Width={0}; Height={1}; Color Depth={2}\n".format(self.width, self.height, self.color_depth);
...
class ImageWriter:
	...
	def get_bit_availability(self, bitmask):
		metadata_bits = 16 + 16;
		return (self.image.width * self.image.height * self.image.color_depth - 2) * bitmask - metadata_bits;

	def pack(self, sources, bitmask):
		...
		#self.pixel[self.c] = (self.pixel[self.c] & 0xFE) | ((self.image.color_depth - 3) & 0x01);
		#self.next_pixel_component(1);

		# Metadata length
		self.__embed_str(self.__int16_to_str(0));
...
class ImageReader:
	...
	def __unpack(self, prefix, suffix):
		...
		#transparent = (self.pixel[self.c] & 0x01);
		#self.next_pixel_component(1);
		#if (3 + transparent != self.image.color_depth):
		#	raise ImageReader.Exception("Image does not contain data");
