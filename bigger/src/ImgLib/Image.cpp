...
namespace ImgLib {
	...
	class JPEGStream : public jpge::output_stream {
	private:
		std::vector<unsigned char>* source;
		unsigned int pos;

	public:
		JPEGStream(std::vector<unsigned char>* source) :
			jpge::output_stream(),
			source(source),
			pos(0)
		{
		...
		bool put_buf(const void* Pbuf, int len) {
			for (int i = 0; i < len; ++i) {
				if (this->pos >= this->source->size()) {
					this->source->resize((this->source->size() == 0 ? 1 : this->source->size() * 2));
				}
				(*this->source)[this->pos] = static_cast<const char*>(Pbuf)[i];
				++this->pos;
			}
			return true;
		}
	...
	bool Image :: saveToVectorJpeg(std::vector<unsigned char>* source, jpge::params* params, std::ostream* errorStream) const {
		assert(params != NULL);
		assert(source != NULL);
		...
		// Create
		jpge::jpeg_encoder e;
		JPEGStream stream(source);
		if (params == NULL) {
			e.init(&stream, this->width, this->height, 3);
		}
		else {
			e.init(&stream, this->width, this->height, 3, *params);
		}

		// Encode
		unsigned char* scanline = new unsigned char[this->width * 3];
		for (unsigned int i = 0; i < this->height; ++i) {
			for (unsigned int x = 0; x < this->width; ++x) {
				scanline[x * 3 + 0] = this->getPixel(x, i, 0);
				scanline[x * 3 + 1] = this->getPixel(x, i, 1);
				scanline[x * 3 + 2] = this->getPixel(x, i, 2);
			}

			e.process_scanline(scanline);
		}
		e.process_scanline(NULL);
		source->resize(stream.getLength());
