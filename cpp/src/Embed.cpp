...
bool fullOptimizeEncode(Image* image, lodepng::State* state, std::vector<unsigned char>* pngOutput, ostream* statusStream, ostream* errorStream);
...
int main(int argc, char** argv) {
	...
	if (sources.size() == 0 && imageFile.length() == 0) {
		cout << "Usage:" << endl;
		cout << "    " << argv[0] << " [-b ...] [-s ...] [-o ...] [-r ...] [-a ...] [-O ...] [-S ...] image.png file1.txt file2.txt ..." << endl;
		cout << "" << endl;
	...
	lodepng::load_file(pngFile, imageFile.c_str());

	// Decode
	cout << "Decoding image..." << endl;
	Image image;
	stringstream errorStream;
	if (!image.loadFromSource(&pngFile, channelCount, &errorStream)) {
	...
	if (sources.size() == 0) {
		...
		for (unsigned int cc = 3; cc <= 4; ++cc) {
			...
			cstring units;
			for (int i = 1; i <= 8; ++i) {
				double byteSpace = (iw.getBitAvailability(i, cc, 0, scatter) / 8);
	...
	while (filesizeLoop) {
		...
		// Bitmask
		if (bitmask == 0) {
			for (bitmask = 1; bitmask <= 8 && (bitRequirement > iw.getBitAvailability(bitmask, channelCount, 0, scatter)); ++bitmask);
			if (bitmask > 8) {
				cout << "  Error: cannot embed files in the image given." << endl;
				cout << "    Max Available: " << iw.getBitAvailability(8, channelCount, 0, scatter) << " bytes" << endl;
		...
		if (fullOptimize) {
			statusStream.str("");
			errorStream.str("");
			if (!fullOptimizeEncode(&image, &state, &pngOutput, &statusStream, &errorStream)) {
		...
		if (filesizeLimit > 0) {
			if (outputSize > filesizeLimit) {
				...
				if (randomizeAll) {
					...
					// Turn on scatter
					if (!scatter) {
						cout << "  Warning: changing scatter from \"off\" to \"on\"" << endl;
						scatter = true;
					}
					else {
						if (bitmask < 8) {
